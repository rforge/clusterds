...
the topology has a single spout which reads data from a static data.frame.
\end{description}
...
\begin{enumerate}
    ...
    look-up.  Micro-clusters were introduced by \cite{stream_clust:Aggarwal:2003} 
    based on the idea of cluster features developed for clustering
    large data sets with the BIRCH algorithm~\cite{stream:Zhang:1996}.
    Micro-clusters
...
fading strategy introduced first for DenStream by~\cite{stream_clust:Cao:2006}. 
Micro-cluster weights are faded in every time step by a factor of $2^{-\lambda}$, 
where $\lambda >0$ is a user-specified fading factor. Since new data points 
have by definition the weight of one, the new data points have more impact on 
the clustering and the effect of older points gradually disappears.
Other models using sliding or landmark windows are discussed in 
...
benchmark problems including splitting and joining clusters, 
...
DST algorithms are implemented in \pkg{stream} as mutable objects.
...
or the recently introduced reference class construct (see package~\pkg{methods}~/cite{stream:R:2005}).
...
since \pkg{stream} currently focuses on this task and has no implemented
algorithms for the other tasks.

Data stream clustering algorithms are implemented 
as subclasses of the class \code{DSD} (see Figure~\ref{figure:dst}).
First we differentiate between different interfaces for clustering algorithms.
\code{DSC_R} provides a native interface, while \code{DSC_MOA} 
...
clusters in the DSC object \code{x}. How the weight
is calculated depends on the clustering algorithm. Typically it is a function of 
the number of points assigned to each cluster. 

\item
\code{get_assignment(dsc, points, type=c("auto", "micro", "macro"), method="Euclidean", ...)} 
...
\code{"plot"}, \code{"pc"}. Method \code{"pairs"} is the default method and produces a
matrix of scatter plots that plots all attributes against one another (this
method defaults to a regular scatter plot for \code{d = 2}). Method \code{"plot"} takes the 
...
Note, that DSC classes are implemented as mutable objects
...
a tree structure for faster access. Either $k$-means or reachability from DBSCAN is used for reclustering.
...
\item\code{DSC_tNN} implements the simple data stream clustering algorithm called threshold nearest-neighbors \citep{stream:Hahsler+Dunham:2014,stream:Hahsler+Dunham:2010b}.
Micro-clusters are defined by a fixed radius around their center.
The radius defines a threshold where new data points are assigned to a micro-cluster 
if they fall within the radius threshold. If a new data point is not close enough
to any micro-cluster, then a new micro-cluster is created.
...
\code{epsilon}-neighborhood). Two micro-clusters are reachable if they are connected
