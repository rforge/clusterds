...
reclustering strategies. For DenStream weighted DBSCAN is used. For CluStream
we use weighted $k$-means clustering and for D-Stream finding 
dense grid clusters is used.
