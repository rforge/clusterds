...
\title{Introduction to \pkg{stream}: An extensible Framework for Data Stream Clustering Research}
...
clustering algorithms. \pkg{stream} is a general purpose tool that includes
...
batch processing. Streaming in the name only means that it uses internally pipes
...
\cite{stream:Urbanek:2014}. Furthermore, \pkg{stream}
...
changes in density and size. This generator is indented to grow into
...
keyframes (similar to keyframes in animation and filmmaking) of functions. 
\item\code{DSD_RandomRBFGeneratorEvents} (\pkg{streamMOA}),
 a data generator using radial base functions with noise. Clusters can
 merge and split.
...
In this case the evaluation measures sightly decrease, since a 
few data points
in the fringes of the clusters are close
to the correct micro-cluster, but at the same time are closer to
to the wrong macro-cluster center.
