...
provides:
\begin{itemize}
\item\code{DSD_ReadCSV}, which reads data from files or open connections
...
\begin{itemize}
\item \code{DSD_Memory} wraps static matrix-like data (e.g., a data.frame, a matrix)
...
\item
\code{update(dsc, dsd, n=1)} which accepts a DSC object and a DSD object. It takes $n$
...
dsd_file <- DSD_ReadCSV(gzfile(file),take=c(1, 5, 6, 8:11, 13:20, 23:41), 
assignment=42, k=7)
dsd_file
@
...
% put them in a \code{DSD_Memory} to make sure that
...
% <<>>= 
% library("stream") 
% dsd <- DSD_Memory(DSD_Gaussians(k=3, d=2, noise=0.01), n=1000)
% dsd
% @
% 
...
% 
% <<>>=
% update(sample, dsd, 1000) 
% sample
% reset_stream(dsd) 
% update(dstream, dsd, 1000)
% dstream
% reset_stream(dsd) 
% update(denstream, dsd, 1000) 
% denstream
% reset_stream(dsd) 
% update(clustream, dsd, 1000)
% clustream
% @
% 
% <<>>=
