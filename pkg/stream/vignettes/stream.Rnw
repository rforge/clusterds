...
\item\code{DSD_mlbenchData}, a DSD that wraps data sets found within the \code{mlbench} package.
...
\code{DSD_Gaussians} specifying the number of clusters as $k=4$,
...
the resulting plot. 
The assignment value is automatically used to change the color in the 
...
The animation is recoded using package \pkg{animation} and can 
be replayed and saved using \code{ani.replay()}.
...
\caption{Plotting the micro-clusters produced by BIRCH together with the
...
and the BIRCH clustering objects 
...
The dynamic clustering can also be visualized using \code{animate_cluster()}.
Animations can be replayed and saved using the package \pkg{animation}.
<<eval=FALSE>>=
...
We can plot the resulting clustering which is shown in Figure~\ref{figure:recluster}.
...
\begin{figure} 
\centering
\includegraphics[width=.5\linewidth]{stream-recluster}
...
uses the micro clusters. For evaluation, $n$ new data points are 
requested from the data stream and each is assigned to its neares micro-cluster.
...
library("stream") 
set.seed(1000) 
d <- get_points(DSD_Gaussians(k=3, d=2, noise=0.01), n=1000,
assignment=TRUE) 
head(d)
@
...
Next, we create the thre clustering algorithms and cluster the same 
...
For D-Stream there is an alternative visualization which shows the density 
of the used grid.
...
\begin{minipage}[b]{.48\linewidth} \centering
\includegraphics{stream-dstream} \\(c) D-Stream 
\end{minipage}
\begin{minipage}[b]{.48\linewidth} \centering
\includegraphics{stream-dstream2} \\(d) D-Stream grid 
\end{minipage}
\caption{Plotting three sets of different micro-clusters 
against the generated data} 
\label{figure:denstream+clustream} 
\end{figure}

Next, we will recluster the micro-clusters generated by DenStream,
CluStream and D-Stream. Different reclustering algorithms were suggested by the
creators of each of the algorithm. 
Althoug, any off-line reclustering strategy can be applied to any
on-line clustering algorithm, 
we will apply here the suggested
reclustering strategies. For DenStream weighted DBSCAN is used. 
Thew parameter eps (reachability distance) is choosen the epsilon-neighborhood
parameter of DenStrean.
For CluStream
we use weighted $k$-means clustering with
$k$ as the actual number of clusters in the data set.
For D-Stream finding 
dense grid clusters is used. Note that for D-Stream we have we have to add a
small amount to to the gridsize so that the single-link hierarchical clustering
joins adjacent dense grids.
...
\includegraphics{stream-denstream_dbscan} \\(a) DenStream with weighted DBSCAN 
\end{minipage}
\begin{minipage}[b]{.48\linewidth} \centering
\includegraphics{stream-clustream_kmeans} \\(b) CluStream with Weighted $k$-means
\end{minipage}

\begin{minipage}[b]{.48\linewidth} \centering
\includegraphics{stream-dstream_grids} \\(c) D-Stream with CluStream with dense 
grid clusters
\end{minipage}
