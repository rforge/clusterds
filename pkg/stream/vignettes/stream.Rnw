...
\title{Introduction to \pkg{stream}: An extensible Framework for Data Stream Mining Research}

\Plainauthor{Matthew Bolanos, John Forrest, Michael Hahsler} 
\Plaintitle{Introduction to stream: An Extensible Framework for Data Stream Mining Research}
...
various data stream mining tasks. In this paper we focus on data stream clustering.
However, the framework is designed to be extended to other stream mining tasks 
in the future.
} 
...
\pkg{birch}~\citep{stream:Dealing:2012} implements
BIRCH, a clustering algorithm for very large data sets. The algorithm
maintains a clustering feature tree which can be updated in a streaming 
fashion. Although BIRCH was not developed as a data stream clustering algorithm,
it still has most characteristics needed for data streams.
...
batch processing. Stream in the name only means that it uses internally pipes
for ``streaming'' data between the Hadoop framework and \proglang{R} code.
A distributed framework for realtime computation is Storm~\cite{stream:Storm:2014}. 
Storm builds on the
...
the topology has a single spout which contains a data.frame.
\end{description}
...
Although, \pkg{stream} currently only implements algorithms for clustering,
we will provide pointers to other popular data stream mining tasks. 
We plan to add algorithms for classification and frequent pattern mining 
in the near future.
...
difficult and involves copying data manually.
...
Figure~\ref{figure:dsd} shows relationship (inheritance) hierarchy of the DSD
...
\begin{itemize}
\item\code{DSD_BarsAndGaussians}, a DSD that generates two bars and two Gaussians clusters with different density.
\item \code{DSD_Gaussians}, a DSD that generates static cluster data with a
random Gaussian distribution. 
\item\code{DSD_mlbenchData}, a DSD that wraps data sets found within the 
\code{mlbench}~\citep{stream:Leisch:2010} package.
\item\code{DSD_mlbenchGenerator}, a class uses the generators for artificial data sets defined in 
...
with concept drift. The behavior of each cluster over time (changes in
position, density and dispersion) can be specified 
using keyframes of functions. 
...
can also be wrapped in a stream class and to be replayed as a stream over and
over again:
\begin{itemize}
...
\item A data generation function \code{get_points(x, n=1, ...)}. 
...
Following the simple interface, 
...
It is important to note that the concept of the DST base class is merely 
for conceptual purposes, the actual implementation 
of clustering (DSC), classification (DSClassify) or frequent pattern mining (DSFP)
are typically quite different and share only basic management functionality.    
...
procedure implemented and this also allows the user to retrieve macro-clusters.
...
\item
\code{plot(x, dsd=NULL, ..., method="pairs", type = c("auto", "micro", "macro", "both")} 
...
from the object after clustering. Note, that DSC classes are implemented as reference classes (mutable objects)
...
The implementations for DSC are split into \proglang{R}-based and 
MOA-based implementations (see \code{DSC_R} and \code{DSC_MOA} in Figure~\ref{figure:dst}). The following clustering algorithms 
are currently available:

\begin{itemize}
\item\code{DSC_BIRCH}, the first pass of the BIRCH (balanced iterative reducing and clustering using hierarchies) algorithm \citep{stream_clust:Zhang:1996}. used to generate a CF tree where the leave notes are used as micro-clusters. 
%\item
%StreamKM++ \citep{stream:Ackermann+Lammersen+Maertens+Raupach:2010}
\item\code{DSC_CluStream}, the CluStream algorithm from MOA \citep{stream_clust:2003}.
\item\code{DSC_ClusTree}, the ClusTree algorithm from MOA \citep{stream:Kranen+Assent+Baldauf+Seidl:2009}.
\item\code{DSC_DenStream}, the DenStream algorithm from MOA \citep{stream_clust:2006}.
\item\code{DSC_DStream}, the D-Stream algorithm \citep{stream:Tu:2009}.
%\item
...
\begin{itemize}
\item \code{DSC_DBSCAN}, DBSCAN~\citep{Ester96adensity-based}.
\item \code{DSC_Hierarchical}, interface to R's \code{hclust} function.
\item \code{DSC_Kmeans}, interface to \proglang{R}'s $k$-means implementation or a version of $k$-means where the data points (micro-clusters) are weighted by the micro-cluster weights, i.e., a micro-cluster representing more data points has more weight.
...
Stream also supports data streams which contain concept drift. An example
for such a data stream generator is MOA's
\code{DSD_RandomRBFGeneratorEvents} where clusters move over time.
...
To show concept drift, we request four times 500 data points from the stream and 
plot them. 
...
\includegraphics[width=\linewidth]{stream-moa1} \\(a) Position 500 
\end{minipage}
\begin{minipage}{.48\linewidth} \centering
\includegraphics[width=\linewidth]{stream-moa2} \\(b) Position 1000
\end{minipage} \\
\begin{minipage}{.48\linewidth} \centering
\includegraphics[width=\linewidth]{stream-moa3} \\(c) Position 1500
\end{minipage}
\begin{minipage}{.48\linewidth} \centering
\includegraphics[width=\linewidth]{stream-moa4} \\(d) Position 2000
\end{minipage}
...
Next, we prepare the clustering algorithm. We use here \code{DSC_DenStream}
and set the 
size of the epsilon neighborhood 0.1.
...
<<>>= 
cluster(denstream, dsd, 500) 
denstream
@

After clustering 500 data points, the clustering contains
\Sexpr{nclusters(denstream)} micro-clusters. 
Note that our implementation of DenStream has reclustering built in and therefore 
also shows macro-clusters.
The micro-cluster centers
are:
<<>>=
head(get_centers(denstream))
@
...
<<>>= 
evaluate(denstream, dsd, n = 500)
@

The number of points used for the evaluation are passed on as the
argument $n$.
Note that the clustering only contains micro-clusters. Therefore 
\code{numMacroClusters} shows \code{NA}.

Individual measures can be calculated using the method argument.

<<>>=
evaluate(denstream, dsd, method = c("purity", "crand"), n = 500)
@
...
ground truth)
over 5000 data points in the data stream. Purity drops between points 2000 and 
3500 because two clusters in the data stream overlap significantly 
during that time. 
...
with a macro-cluster. Some supported
...
\end{minipage}
\caption{A data stream clustered with CluStream and then reclustered with
...
uses the macro-clusters. For evaluation, $n$ new data points are 
...
In this case the evaluation measures sightly decrease, since for a 
few data points
in the fringes of the clusters are closer to a different macro-cluster.
...
%<<dstream, fig=TRUE, include=FALSE>>= 
%reset_stream(dsd) 
%plot(dstream, dsd, xlim=c(0,1), ylim=c(0,1)) 
%@
...
reclusters by merging adjacent dense grids. Both clustering algorithms 
already produce macro-clusters and can be directly used for analysis.
CluStream does not have reclustering built in, since the extra parameter
$k$ for weighted $k$-means is needed. We apply the reclustering with the following
two lines.
...
reset_stream(dsd) 
plot(clustream_kmeans, dsd, type="macro") 
@
...
FUN=function(x){
    ...
    evaluate(x, dsd, c("numMicroclusters", "numMacroclusters", 
      "purity", "crand"), type="macro", assign="micro")
})
@
...
From the evaluation results we see that CluStream with weighted $k$-means
reclustering performs best. 
But this is only due to the fact that it is the only
algorithm that produces three clusters. The density-based
algorithms (DenStream and D-Stream) are not able to separate the overlapping 
clusters. CluStream only separates them correctly, because we set $k=3$ for
reclustering.

Clustering and reclustering algorithms can be mixed and matched as needed to evaluate
