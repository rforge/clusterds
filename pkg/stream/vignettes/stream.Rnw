...
\subsection{Other Data Stream Mining Tasks} \label{sec:background:dscl}
...
\subsection{Existing Solutions} \label{sec:background:moa}
...
interfaces with WEKA, provides already a large set of data stream classification and
...
using nearest-neightbor assignment (\code{"nn"}). If the assignment method is set to 
\code{"auto"} then model assignment is used when avaialble and otherwise 
nearest-neighbor assignemnt is used.
...
In the following we will enumerate the evaluation measures (passed on as \code{measure}) availabe in
...
or in the documentation supplied with the packages \pkg{cluster}, \pkg{clue} and \pkg{fpc}.
...
\item Internal evaluation measures
	\begin{itemize}
	  ...
	  \item \code{"ave.within.cluster.ss"} a generalisation of the within clusters sum of squares (half the sum of the within cluster squared dissimilarities divided by the cluster size) (\pkg{fpc}),
	  \item \code{"g2"} Goodman and Kruskal's Gamma coefficien (\pkg{fpc}), 
...
%		\item	\code{"classPurity"} (of real clusters; see Wan et al (2009)), 
    ...
    %(see Rand (1971)) 
    (\pkg{clue}),
		\item	\code{"cRand"} Adjusted Rand index 
...
and in addition to the methods sum of squared (\code{"SSQ"}) and
...
We show how to cluster data streams in Section~\ref{examples:cluster_ds} and
...
the data dimensionality of $d=3$ with an added noise of 5\% of the generated
...
in file \code{DSD_UniformNoise.R}. This generator creates points
...
For the \proglang{R} interface, the clustering class (S3) needs to contain
...
a reference class object. Reference classes have been
...
RObj needs to contain the following methods:
\begin{enumerate}
...
\item For macro-clusters: \code{get_macroclusters(...)}, \code{get_macroweights}
and \code{microToMacro(micro, ...)} which does micro- to macro-cluster
matching.
\end{enumerate}

Note that these are methods for reference classes and this do not contain the
