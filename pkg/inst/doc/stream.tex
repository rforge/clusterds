...
Because we have only defined two of the parameters, \code{mu}, \code{sigma}, \code{p}, and \code{noise} will be left to their default values. The \code{print()} function displays a brief summary of the DSD object.
...
%%TODO: talk to dr. hahsler about how we want to plot tihs
%%<<plot5, fig=TRUE, include=FALSE>>=
%%plot(dbscan(get_centers(dsc1), 0.3), d)
%%points(dbscan(get_centers(dsc2), 0.3), d)
%%@

Figure~\ref{figure:plot5} shows the same data, plotted against the macro clusters calculated by the \textit{DBSCAN} algorithm from the \pkg{fpc} package \citep{stream:Hennig:2010}.
...
There are several external packages that are required to use the \pkg{stream} package. These include the \pkg{proxy} package, written by \cite{stream:Meyer+Buchta:2010}, the \pkg{MASS} package by \cite{stream:Venables+Ripley:2002}, and \pkg{clusterGeneration} by \cite{stream:Qiu+Joe:2009}. To facilitate the communication between \proglang{R} and \proglang{Java}, we used the \pkg{rJava} \citep{stream:Urbanek:2010}. This allowed us to make method calls directly to the JRI from within the \proglang{R} environment. The \pkg{stream} Reference Manual is available in the appendix. It documents all of the available classes and functions, as well as the details behind their implementation.
