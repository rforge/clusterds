...
\item\code{DSD_Target}, a DSD that generates the target shape.
...
\item
\code{get_microclusters(x, ...)} which returns the micro-clusters, 
either the centroids or the medoids, when a macro-cluster is used.
If the DSC is a micro-clusterer then the centers are returned.
