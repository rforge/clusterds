...
The resulting clustering is a static copy (\code{DSC_Static}) and further clustering
cannot be performed with it. Reclustering, however,  can be performed on it.

\section{Evaluating Data Stream Mining}
Evaluation of data stream mining is an important issue. We will only discuss 
...
\section{Evaluating Data Stream Clustering}
...
Evaluation in \pkg{stream} is performed via 
\centerline{\code{get\_evaluation(dsc, dsd, method, n = 1000, type=c("auto", "micro", "macro"), assign="micro"}.}
\code{n} data points are taken from \code{dsd} and assigned to their closest cluster in the 
clustering 
in \code{dsc} using Euclidean distance. 
By default the points are assigned to micro-clusters (\code{assign}).
Then the assignments are aggregated to the level specified in \code{type}.
So for a macro-clustering, the assignments will be translated into 
...
Most evaluation measures require the ground truth (class label) for the data (\code{dsd}).
If this is available then for each cluster the majority class in the assigned data points
is found and is used as the class label for the cluster.
Based on 
the majority class and the class of all assigned points several 
evaluation measures are available. 
We will not describe each measure, since most of them are standard measures 
which can be found in many text books \citep[e.g.,][]{clust:Jain:1988,Kaufman:1990}.
The following evaluation measures are currently implemented (method name used in 
\code{get_evaluation} are typed as code):
\begin{itemize}
	    \item   \code{"precision"} 
	    \item   \code{"recall"}
	    \item   F1 measure (\code{"F1"}), 
	    \item   \code{"purity"}
	    \item   false positive rate (\code{"fpr"})
	    \item   Rand index (\code{"Rand"})
	    \item   Adjusted Rand index (\code{"cRand"}),
...
reference classes use a completely different object-oriented
...
get_evaluation(tnn, dsd, n = 500)
@

Individual measures can be calculated using the method argument.
<<>>=
get_evaluation(tnn, dsd, method = c("f1", "rand"), n = 500)
@
...
Finally, we can compare the clusterings using \code{get_evaluation}.

<<>>= 
sapply(list(DenStream=km_denstream, CluStream=km_clustream), 
FUN=function(x){
    reset_stream(dsd)
    get_evaluation(x, dsd, c("purity","rand", "crand"))
})
@
