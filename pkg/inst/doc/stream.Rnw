...
micro-cluster flag to true. Lastly, the size of the centers can mirror the weights of each
...
micro <- DSC_tNN(r=.04, macro=FALSE)
...
reset_stream(dsd) 
plot(dsc_denstream,dsd) 
@

<<clustream, fig=TRUE, include=FALSE>>= 
reset_stream(dsd) 
plot(dsc_clustream,dsd) 
@
...
reset_stream(dsd) 
plot(macro_denstream,dsd) 
@

<<clustreamkmeans, fig=TRUE, include=FALSE>>= 
reset_stream(dsd) 
plot(macro_clustream, dsd) 
@
...
<<>>= 
sapply(list(DenStream=macro_denstream, CluStream=macro_clustream), 
FUN=function(x){
    reset_stream(dsd)
    get_evaluation(x,dsd,c("f1","precision","recall","rand"))
})
@
