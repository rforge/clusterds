...
<<>>= 
recluster <- DSC_Kmeans(centers=3)
centers <- get_centers(dsc)
dsd_temp <- DSD_Wrapper(centers)
cluster(recluster,dsd_temp,nrow(centers))
@
...
<<>>= 
get_evaluation(recluster, dsd, method = "f1", n = 1000)
@
...
batch_Clusters(list(dsc1,dsc2), dsd, recluster=list(recluster), measures=c("f1","recall","precision"), input_n=1000)
@
