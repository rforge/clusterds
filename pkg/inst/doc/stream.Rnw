...
\begin{enumerate}
    ...
    non-empty grid cells (e.g.,~\cite{stream:Tu:2009, stream:Wan:2009}).
...
\begin{center}
\code{get\_evaluation(dsc, dsd, method, n = 1000, type=c("auto", "micro", "macro"),}\\
\code{assign="micro")},
\end{center}
...
\code{evaluate} are typed as code:
\begin{itemize}
	    \item   \code{"precision"} 
	    \item   \code{"recall"}
...
Furthermore, the presented infrastructure can be extended by adding new algorithms. We have abstracted each component to only require a small
...
Currently, \pkg{stream} only includes implementations for the data stream 
clustering tasks. In the future we plan to also incorporate 
