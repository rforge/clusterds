...
(\code{DSC_R}) and MOA-based implementations (\code{DSC_MOA}).
...
\code{DSD_MOA} creates dynamic data streams where clusters move over time.
...
the beginning of the stream (\code{reset_stream()} is also available for
