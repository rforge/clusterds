...
%%TODO
The DST interface requires more work in that there are currently 2 abstract classes that extend directly from the abstract base class, \code{DataStreamClusterer}.
...
We can also create streams with dynamic data by using the \code{DSD_MOA} class. Figure~\ref{figure:moa} shows the concept drift in \code{DSD_MOA} as the initial 3 clusters move around, and finally merge with one another.
...
<<>>=
dsc <- DSC_DenStream()
d <- get_points(DSD_MOA(k=3, d=2, modelSeed=100, instanceSeed=100), 3000)
head(d)
