...
proposed regularely and it is important to evaluate them 
thouroughly under standardized conditions.
...
\cite{stream_clust:Guha:2003,
    ...
    stream_clust:Tu:2009,
    stream_clust:Wan:2009,
...
\begin{enumerate}
    ...
    lookup.  Micro-clusters are representatives for sets of similar data points
    ...
    non-empty grid cells (e.g.,~\cite{stream_clust:Tu:2009, stream_clust:Wan:2009}).
...
interfacs with WEKA, provides already a set of data stream classification and
clustering algorithms and it
provides a clear Java interface to add 
new algorihtms or use the existing algorithms in other applications.
...
(hyper) cube.
\item\code{DSD_mlbench}, a class uses the generators for artificial datasets defined in 
the \code{mlbench} package.
\item\code{DSD_Target}, a DSD that generates a ball in circle data set.
\item\code{DSD_Data}, a DSD that wraps datasets found within the \code{mlbench} package.
...
\marginpar{add bars and Gaussians data}
...
\item The data generation function \code{get_points(x, n=1, ...)}. This function is used
...
DSCs implement the online process (called \code{DSC_Micro} since it produces micro-clusters)
and the offline process (called \code{DSC_Macro}).

\marginpar{macro\_clusters should have their own list}
...
micro-clusters will be returned while final clusters will
be returned for a \code{DSC_Macro}.

\item
\code{get_weights(x)} which returns the weights of the
clusters (e.g., number of points assigned to the cluster) in the DSC object.
...
In addition,
\code{cluster_animation(dsc, dsd, n=1, ..., outdir=NULL)} can be used to produce an animation of the clustering process. If the \code{outdir} is set then an animated gif will be produced.
...
results via \code{get_centers()}.

\marginpar{add recluster}
...
are avaialble as objects of class \code{DSC_Macro}:
\begin{itemize}
\item
...
\section{Evaluating Data Stream Mining}
\marginpar{missing}
...
For a new DSD implementation only a creator function and \code{get_points()}
...
rows. Also, if the ground truth (true cluster assignemnt) for the data is 
avaialble, then this can be attached to the data.frame as an attribure
calles \code{"assignment"} as an integer vector (noise typically is represented
by \code{NA}).
For a complete example, look at the implementaion 
...
for an element called \code{"RObj"},
 which needs to be a mutable R object containing the clustering.   
...
Mutability can be achieved in R by using the environment mechanism and 
was introduces recently with R-2.12 as its own construct in the 
form of reference classes
in package \pkg{methods}.
The RObj in DSC is expected to be a reference class with \code{cluster} method.
...
For example, the cluster method of Robj is invoced by \code{RObj$cluster()}.
...
instruct the reference class object to update its clustering while 
For a complete example, look at the implementaion 
of \code{DSD_tNN} in the package's source code.
...
If the clustering also supports weights, then a \code{get_weights()}
...
\marginpar{set seed does not work for Java!}
...
reseting the position in the \code{DSD_ReadStream} to the file's beginning
...
data (e.g., an anomality) and then adapt our algorithm and test if the 
...
indefinately, i.e., start over once the last data point is reached.
This is achived by passing \code{loop=TRUE} to the creator function.
The current position in the stream for those 
two types of DSD classes can also be reset to the begining of the stream
...
and set the 
radius (BIRCH's closeness criterion) to 0.1.

<<>>=
dsc <- DSC_BIRCH(radius=0.1) 
dsc
@
...
Rand index (rand, rand\_HA, rand\_MA, rand\_FM),
within cluster sum of squares (ssq), and the informational items
number of clusters (numCluster) and
...
get_evaluation(dsc,dsd, n = 500)
@

Individual measures can be calculated using the method argument.
<<>>=
get_evaluation(dsc,dsd, method = c("f1", "rand"), n = 500)
@

\marginpar{evaluate measures over time}
...
<<>>= 
library("stream") 
d <- get_points(DSD_GaussianStatic(k=3, d=2, noise=0.01), 1000,
assignment=TRUE) 
head(d)
...
\code{DSD_GaussianStatic} object so that we can replay the same stream data
