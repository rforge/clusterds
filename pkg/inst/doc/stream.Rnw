...
\item
\code{cluster_animation(dsc, dsd, n = 1, interval = .1, horizon = 500, pointInterval = 100, weights = FALSE, scale = c(1,10), outdir = NULL, ...)} which animates the clustering process. The parameters alter the presentation of the animation. If the \code{outdir} is set
...
\item
\code{plot(x, dsd = NULL, col_points = "gray", col_macro = "red", col_micro = "black", weights = FALSE, scale = c(1,10), ..., method="pairs", microclusters = FALSE)} which plots the centers
of the centers. There are 3 available plot methods: \code{pairs},
\code{plot}, \code{pc}. If a DSD is given then the centers will be overlayed on top, and if
...
<<>>= 
library("stream") 
d <- get_points(DSD_Gaussian_Static(k=3, d=2, noise=0.01), 10000,assignment=TRUE) 
head(d)
...
reset_stream(dsd) 
plot(dsc1,dsd, xlab='x', ylab='y') 
@

<<denstream, fig=TRUE, include=FALSE>>= 
reset_stream(dsd) 
plot(dsc2,dsd, xlab='x', ylab='y') 
@
...
<<>>= 
macro1 <- DSC_KmeansW(3)
macro2 <- DSC_KmeansW(3)
recluster(macro1,dsc1)
recluster(macro2,dsc2)
@

<<denstreamkmeans, fig=TRUE, include=FALSE>>= 
reset_stream(dsd) 
plot(macro1,dsd, xlab='x', ylab='y',weights=TRUE) 
@

<<clustreamkmeans, fig=TRUE, include=FALSE>>= 
reset_stream(dsd) 
plot(macro2,dsd, xlab='x', ylab='y',weights=TRUE) 
@
...
<<>>= 
reset_stream(dsd)
get_evaluation(macro1,dsd,c("f1","precision","recall","rand"),1000)
reset_stream(dsd)
get_evaluation(macro2,dsd,c("f1","precision","recall","rand"),1000)
@
