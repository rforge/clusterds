cluster_animation <- function(dsc,dsd,n=1,interval=.1, pointInterval=100, horizon=5*pointInterval, evaluationMethod=NULL, outdir=NULL,...) {
	if(is.null(outdir)) {
		cluster.ani(dsc, dsd, n, interval, pointInterval, horizon, evaluationMethod, save=FALSE,...)
	} else {
		saveMovie(cluster.ani(dsc, dsd, n, interval, pointInterval, horizon, evaluationMethod, save=TRUE,...), interval = interval, outdir = outdir)
	}
}
...
cluster.ani <- function(dsc=NULL, dsd, n,interval=.1, pointInterval=100, horizon=5*pointInterval, evaluationMethod=NULL, save=TRUE, ...) {
